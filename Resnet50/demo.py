...
def demo_webcam(model, idx_to_class, face_mesh, camera_id=0):
    ...
    while True:
        ...
        cv2.imshow("Face Recognition Demo (ESC to exit)", frame)
...
def main():
    ...
    with mp_face_mesh.FaceMesh(static_image_mode=args.image is not None, max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5) as face_mesh:
        if args.image:
